...
features_lst = ["Location", "Acct#", "Current Assessment", "Building Count", 
...
def add_house_data(URL):
  global df
  house_page = requests.get(URL, verify=False)
...
def main():
  ...
  for i in street_letters: 
    ...
    for anchor_tag in links: #anchor_tag is a tag such as <a href="Streets.aspx?Name=LEE RD">LEE RD</a>
      ...
      house_link_tags = strt_html_soup.find_all("a", href=True)

      #Remove non-house links
      house_link_tags = filter(has_link_to_house, house_link_tags) #each tag is a tag such as <a href="Parcel.aspx?pid=123">10 LEE RD</a>
      house_pids = [anchor_tag["href"] for anchor_tag in house_link_tags] #anchor_tag["href"] returns the pid part of the tag (our URL route)
      house_URLS = [f"https://gis.vgsi.com/lexingtonma/{house_pid}" for house_pid in house_pids]
      #add_house_data(houseURL)

      num_threads = min(MAX_THREADS, len(house_URLS))
      #map(add_house_data, house_URLS) #without multithreading
